...
$sql = "
    SELECT id, usuario, nombre, apellido, jerarquia, rol, activo, fecha_creacion
    FROM usuarios
    WHERE id = ?
    LIMIT 1
";
...
if (!mysqli_stmt_execute($stmt)) {
    die("Error al consultar el usuario: " . mysqli_stmt_error($stmt));
}
...
if (!$usuario_actual) {
    die("El usuario solicitado no existe.");
}
...
$datos_anteriores = [
    ...
    'jerarquia' => $usuario_actual['jerarquia'],
    'rol' => $usuario_actual['rol'],
    'activo' => (int)$usuario_actual['activo']
];
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($usuario === "") {
        ...
        $sql_verificar = "SELECT id FROM usuarios WHERE usuario = ? AND id != ? LIMIT 1";
        $stmt_verificar = mysqli_prepare($conexion, $sql_verificar);
        mysqli_stmt_bind_param($stmt_verificar, "si", $usuario, $id);
        ...
        if ($usuario_existente) {
            $error = "El nombre de usuario '" . $usuario . "' ya se encuentra registrado por otro usuario.";
        } else {
            ...
            if ($password !== "") {
                ...
                $sql_update = "
                    UPDATE usuarios
                    SET usuario = ?, nombre = ?, apellido = ?, jerarquia = ?, rol = ?, activo = ?, password = ?
                    WHERE id = ?
                ";
                $stmt_update = mysqli_prepare($conexion, $sql_update);
                mysqli_stmt_bind_param($stmt_update, "sssssisi", $usuario, $nombre, $apellido, $jerarquia, $rol, $activo, $password_hash, $id);
            } else {
                // Actualizar sin contraseña
                $sql_update = "
                    UPDATE usuarios
                    SET usuario = ?, nombre = ?, apellido = ?, jerarquia = ?, rol = ?, activo = ?
                    WHERE id = ?
                ";
                $stmt_update = mysqli_prepare($conexion, $sql_update);
                mysqli_stmt_bind_param($stmt_update, "sssssii", $usuario, $nombre, $apellido, $jerarquia, $rol, $activo, $id);
            }

            if ($stmt_update === false) {
                $error = "Error SQL al preparar la actualización: " . mysqli_error($conexion);
            } else {
                ...
                if (mysqli_stmt_execute($stmt_update)) {
                    ...
                    auditar(
                        ...
                        'usuarios',
                        $id,
                        'Se editó el usuario: ' . $usuario . ' (' . $apellido . ', ' . $nombre . ') - Rol: ' . $rol,
                        $datos_anteriores,
                        [
                            'id' => $id,
                            'usuario' => $usuario,
                    ...
                    $usuario_actual["jerarquia"] = $jerarquia;
                    $usuario_actual["rol"] = $rol;
                    $usuario_actual["activo"] = $activo;

                } else {
                    $error = "Error al actualizar el usuario: " . mysqli_stmt_error($stmt_update);
                }
...
<head>
    ...
    <title>Editar usuario - Sistema Medidas</title>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .info-ayuda { background: #fef3c7; border: 1px solid #fcd34d; color: #92400e; padding: 12px; border-radius: 7px; margin-bottom: 15px; font-size: 13px; }
...
<div class="layout">
    ...
    <main class="main">
        ...
        <section class="content">
            ...
            <div class="info">
                <strong>ID del usuario:</strong> <?= (int)$usuario_actual["id"] ?>
                <br>
                <strong>Fecha de alta:</strong> <?= date("d/m/Y H:i", strtotime($usuario_actual["fecha_creacion"])) ?>
            </div>
            ...
            <form method="POST" autocomplete="off">
                ...
                <div class="card">
                    ...
                    <div class="grid">
                        <div class="form-group">
                            ...
                            <input type="text" id="usuario" name="usuario" required maxlength="50" value="<?= htmlspecialchars($usuario_actual["usuario"]) ?>">
                        </div>
                        <div class="form-group">
                            <label for="jerarquia">Jerarquía</label>
                            <input type="text" id="jerarquia" name="jerarquia" maxlength="100" value="<?= htmlspecialchars($usuario_actual["jerarquia"] ?? "") ?>">
                        </div>
                        <div class="form-group">
                            <label for="nombre">Nombre *</label>
                            <input type="text" id="nombre" name="nombre" required maxlength="100" value="<?= htmlspecialchars($usuario_actual["nombre"]) ?>">
                        </div>
                        <div class="form-group">
                            <label for="apellido">Apellido *</label>
                            <input type="text" id="apellido" name="apellido" required maxlength="100" value="<?= htmlspecialchars($usuario_actual["apellido"]) ?>">
                        </div>
                        <div class="form-group full">
                            <label for="rol">Rol del usuario *</label>
                            <select id="rol" name="rol" required>
                                <option value="consulta" <?= ($usuario_actual["rol"] === "consulta") ? "selected" : "" ?>>CONSULTA - Solo puede ver</option>
                                <option value="operador" <?= ($usuario_actual["rol"] === "operador") ? "selected" : "" ?>>OPERADOR - Puede crear cédulas</option>
                                <option value="supervisor" <?= ($usuario_actual["rol"] === "supervisor") ? "selected" : "" ?>>SUPERVISOR - Puede editar cédulas y encargados</option>
                                <option value="admin" <?= ($usuario_actual["rol"] === "admin") ? "selected" : "" ?>>ADMIN - Control total</option>
                            </select>
                            <div class="info-rol">
                        ...
                        <div class="form-group">
                            ...
                            <select id="activo" name="activo" required>
                                <option value="1" <?= ((int)$usuario_actual["activo"] === 1 ? "selected" : "") ?>>ACTIVO</option>
                                <option value="0" <?= ((int)$usuario_actual["activo"] === 0 ? "selected" : "") ?>>INACTIVO</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    <h2 class="card-title">Cambiar contraseña (opcional)</h2>
                    ...
                    <div class="grid">
                        ...
                        <div class="form-group">
                            ...
                            <input type="password" id="password_confirm" name="password_confirm" minlength="6" placeholder="Repita la contraseña">
                        </div>
                    </div>
                </div>
